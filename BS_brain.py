...
import pickle
import keras
from keras.layers import Input, Dense, Layer, activations, Lambda, add,Concatenate
...
class Actor:
    ...
    def __call__(self, state):
        return self.model.predict(state) * self.action_bound
...
class Critic:
    def __init__(self, state_dim, action_dim):
        ...
        self.model.compile(optimizer=Adam(lr=0.002), loss='mse')

    def __call__(self, state, action):
        return self.model.predict([state, action])

class DDPG:
    def __init__(self, state_dim, action_dim, action_bound, actor_lr=0.001, critic_lr=0.002, gamma=0.99, tau=0.005):
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.action_bound = action_bound
        self.gamma = gamma
    ...
    def train(self, replay_buffer, batch_size=64):
        ...
        target_q_values = np.reshape(target_q_values, (batch_size, 1))

        self.critic.model.train_on_batch([states, actions], target_q_values)
        ...
        self.update_target(self.actor_target.model, self.actor.model)
        self.update_target(self.critic_target.model, self.critic.model)
...
class DuelingDQN(Model):
    def __init__(self, num_actions):
        ...
        self.dense1 = Layer.Dense(128, activation='relu')
        self.dense2 = Layer.Dense(128, activation='relu')
        self.state_value = Layer.Dense(1)
        self.advantage = Layer.Dense(num_actions)
...
class BS:
    ...
    def train(self, state, action, reward, next_state, done, batch_size):
        ...
        if len(self.replay_buffer) > batch_size:
            self.ddpg.train(self.replay_buffer, batch_size)

    def act(self, state):
        state = np.expand_dims(state, axis=0)
        action = self.ddpg.actor(state)
        return np.squeeze(action)

    def update_target_model(self):
        self.ddpg.update_target(self.ddpg.actor_target.model, self.ddpg.actor.model)
        self.ddpg.update_target(self.ddpg.critic_target.model, self.ddpg.critic.model)
...
class Agent:
    ...
    def __init__(self, num_d2d, num_ch, num_neighbor, num_d2d_feedback, environment, curr_rl_config):
        ...
        self.model = DuelingDQN(self.num_Actions)
        self.target_model = DuelingDQN(self.num_Actions)
    ...
    def _train_from_memory(self):
            ...
            next_q_value = next_q_values[np.arange(self.batch_size), next_actions]
            ...
            with tf.GradientTape() as tape:
                ...
                loss = self.loss_fn(target_q_value, q_action)

            grads = tape.gradient(loss, self.model.trainable_variables)
            self.optimizer.apply_gradients(zip(grads, self.model.trainable_variables))

            if self.memory.samples % self.update_target_freq == 0:
                self.update_target_model()
