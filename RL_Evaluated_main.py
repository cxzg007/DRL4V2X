...
def main():
    ...
    for test_loop in range(num_test_settings):
        ...
        new_path = r"E:\jie\Globecom2020-ResourceAllocationGNN-main"
        BS_Agent_0 = load_trained_model_0(Env, curr_RL_Config, new_path=new_path)
        
        new_path = r"C:\Users\Huangyi\Desktop\Globecom2020-ResourceAllocationGNN-main"
        BS_Agent_1 = load_trained_model_1(Env, curr_RL_Config, new_path=new_path)
        ...
        save_flag = run_test(curr_RL_Config, BS_Agent_0,BS_Agent_1, test_seed_sequence, Fixed_Epsilon, num_evaluate_trials)
...
def load_trained_model_0(Env, curr_RL_Config, new_path=None):
    ...
    BS_Agent_0 = Agent(Num_d2d, Num_CH, Num_neighbor, Num_D2D_feedback, Env, curr_RL_Config)
    ...
    curr_sim_set = 'Train-Result' + '-RealFB-' + str(Num_D2D_feedback) + '-Batch-' + str(BATCH_SIZE) \
                   + '-Gamma-' + str(GAMMA) \
                   + '-V2Iweight-' + str(V2I_Weight) + '0'

    # Use the new_path if provided, otherwise use the default current working directory
    if new_path:
        folder = os.path.join(new_path, curr_sim_set)
    else:
        folder = os.path.join(os.getcwd(), curr_sim_set)
    ...
    model_para = os.path.join(model_dir, model_name)
    ...
    target_model_para = os.path.join(model_dir, target_model_name)

    # load Q-Function Network weights
    BS_Agent_0.brain.model.load_weights(model_para)
    # load Target Network weights
    BS_Agent_0.brain.target_model.load_weights(target_model_para)

    print('Load the trained model successfully under this setting!')

    return BS_Agent_0

def load_trained_model_1(Env, curr_RL_Config, new_path=None):
    # load the trained RL-DNN model for testing
    Num_neighbor = Env.n_Neighbor
    Num_d2d = Env.n_Veh
    Num_D2D_feedback = curr_RL_Config.Num_Feedback
    Num_CH = Env.n_RB
    # construct a BS agent
    BS_Agent_1 = Agent(Num_d2d, Num_CH, Num_neighbor, Num_D2D_feedback, Env, curr_RL_Config)
    ...
    # load Q-Function Network weights
    BS_Agent_1.brain.model.load_weights(model_para)
    # load Target Network weights
    BS_Agent_1.brain.target_model.load_weights(target_model_para)
...
def run_test(curr_RL_Config, BS_Agent_0,BS_Agent_1, test_seed_sequence, Fixed_Epsilon, num_evaluate_trials):
    ...
    Evaluation_Return_per_Episode_0, Evaluation_Reward_per_Episode,
    RA_Expect_Return, RA_Reward] \
        = BS_Agent_0.evaluate_training_diff_trials(Num_Run_Episodes, Num_Test_Step, Opt_Flag,
                                                Fixed_Epsilon, num_evaluate_trials)
    [Evaluated_Opt_Expect_Return,
        Evaluation_Return_per_Episode_1, Evaluation_Reward_per_Episode,
        RA_Expect_Return, RA_Reward] \
            = BS_Agent_1.evaluate_training_diff_trials(Num_Run_Episodes, Num_Test_Step, Opt_Flag,
                                                    Fixed_Epsilon, num_evaluate_trials)
    ...
    Expect_Return = Evaluation_Return_per_Episode_0
    ...
    Mean_Return_per_Episode_0 = np.mean(Evaluation_Return_per_Episode_0, axis=0)
    Mean_Return_per_Episode_1 = np.mean(Evaluation_Return_per_Episode_1, axis=0)
    ...
    myMean_Return_per_Episode_0 = rl_utils.forward_fill(Mean_Return_per_Episode_0, 50)
    y1 = myMean_Return_per_Episode_0
    myMean_Return_per_Episode_1 = rl_utils.forward_fill(Mean_Return_per_Episode_1, 50)
    y2 = myMean_Return_per_Episode_1
    plt.figure()
    x = my_x_ticks
    plt.plot(x, y1, color='blue',label='Original Algorithm')
    plt.plot(x, y2, color='red',label='Improved Algorithm')
